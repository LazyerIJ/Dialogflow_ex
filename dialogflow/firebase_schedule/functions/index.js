...
//intent for get alarmnotice
app.intent('alarmNotice', (conv,{number}) => {
    var ref = db.ref(myPath + "TopNotice/Notice");
    var userRef = ref.child("공지");
    ...
    return userRef.once('value').then(function(snapshot) {
        snapshot.forEach(function(childSnapshot){
            ...
            if(key===number){
            conv.close(`${key}`+" 번째 공지사항은  "+`${item}`+" 입니다. 까먹지마세요");
            }
        });
        return;
